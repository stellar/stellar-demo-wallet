...
export default function render() {
  return [
    ...
    ? [
      ...
      this._id === 'user_1' ? <button class={this.loading.pay ? 'loading' : null} type="button" onClick={() => this.makePayment()}>{this.loading.pay ? <stellar-loader /> : null} Make FeeBump Payment</button> : null,
    ]
    : <button class={this.loading.fund ? 'loading' : null} type="button" onClick={() => this.createAccount()}>{this.loading.fund ? <stellar-loader /> : null} Create {this._id} Account</button>,
